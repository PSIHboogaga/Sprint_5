...
class TestRegistration:
    def test_registration(self, driver):
        ...
        WebDriverWait(driver, 10).until(
            expected_conditions.element_to_be_clickable(StellarburgersLocators.Http_registr))  # подождали пока ссылка Зарегистрироваться станет кликабельной (важная тема для слабых ПК и медленного интернете)
        ...
        WebDriverWait(driver, 10).until(expected_conditions.element_to_be_clickable(StellarburgersLocators.Vhod))  # подождали пока кнопка станет кликабельной (важная тема для слабых ПК и медленного интернета)
