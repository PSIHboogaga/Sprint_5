...
class TestGoFomProfileInLego:
    def test_go_fom_profile_in_lego(self, driver):
        ...
        driver.find_element(*StellarburgersLocators.Button_self_akk).click()  # Клик кнопки личный кабинет
        WebDriverWait(driver, 10).until(expected_conditions.visibility_of_element_located((By.XPATH, ".//p[text() = 'Конструктор']")))  # подождали пока кнопка конструктор станет кликабельной (важная тема для слабых ПК и медленного интернета)
        driver.find_element(*StellarburgersLocators.Button_lego).click()  # Клик кнопки конструктор
        WebDriverWait(driver, 10).until(expected_conditions.visibility_of_element_located((By.XPATH, ".//h1[text() = 'Соберите бургер']")))  # подождали пока отобразится Соберите бургер на странице (важная тема для слабых ПК и медленного интернета)
